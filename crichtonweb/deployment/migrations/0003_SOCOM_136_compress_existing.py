...
from core.fields import uncompress_string

class Migration(DataMigration):

    def forwards(self, orm):
        print "Compressing logs in mysql, may take a while..."
        for x in orm.ProductDeployment.objects.all():
            x.save() # should auto-compress
        
        for x in orm.ApplicationDeployment.objects.all():
            x.save() # should auto-compress
        
        for x in orm.NodeDeployment.objects.all():
            x.save() # should auto-compress


    def backwards(self, orm):
        print "Uncompressing logs in mysql, may take a while..."
        for x in orm.ProductDeployment.objects.all():
            x.log = uncompress_string(x.log)
            x.save()

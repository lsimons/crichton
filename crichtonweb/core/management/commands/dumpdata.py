...
#
# Modified: 1/28/11
# Leo Simons made it work with django-audit-log LastUserField
# and made the algorithm sort tables so dumps are consistently
# ordered between invocations.
...
def foreign_key_sort(items):
    ...
    roots.sort(cmp=lambda x,y: -1*cmp(str(x), str(y))) # LSD - make slower, but deterministic
    ...
    while len(roots) != 0:
        ...
        children.sort(cmp=lambda x,y: -1*cmp(str(x), str(y))) # LSD - make slower, but deterministic
...
def related_field_filter(lst):
    ...
    for f in lst:
        s = str(f)
        if s.find('django.db.models.fields.related') >= 0 or \
                s.find('audit_log.models.fields.LastUserField') >= 0:
            ret.append(f)

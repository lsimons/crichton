...
# populate the temporary package_obj based on [package_name, version]
class Migration(DataMigration):

    def forwards(self, orm):
        "Write your forwards methods here."
        for re in orm.ReleaseElement.objects.all():
            package_name = re.package.name
            package_version = re.package_version
            package, created = orm['package.Package'].objects.get_or_create(name=package_name, version=package_version)
            re.package_obj = package
            re.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        for re in orm.ReleaseElement.objects.all():
            package_obj = re.package_obj
            package_name_obj, created = orm['package.PackageName'].objects.get_or_create(name=package_obj.name)
            re.package = package_name_obj
            re.package_version = package_obj.version
            re.save()

...
#   ./edit_manifest_package.rb pal pal.pp --parent=core pal-foo=1.0.0 pal-bar=2.1.2
#
#   upgrading a package to 'latest' works too
#   ./edit_manifest_package.rb pal pal.pp --parent=core pal-foo=latest
#
#   adding a new package
#   ./edit_manifest_package.rb pal pal.pp --parent=core pal-new-package=1.0.0
...
def main(classname, manifest, versions, env, parent)
  ...
  tcol.hostclasses.each do |name,c|
    ...
    if parent and parent != ""
      code += "inherits #{parent} "
    elsif c.parent and c.parent != ""
...
if __FILE__ == $0
  ...
  fname = ARGV.shift
  parent = nil
  if ARGV[0].start_with? "--parent="
    parent = ARGV.shift[9..-1]
  end
  #puts "parent", parent
  ...
  env = "production"
  manifest = main(classname, manifest, versions, env, parent)
